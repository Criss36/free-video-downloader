...
原理：从 URL 提取 video_id → 公开 API 获取元数据 → 无水印播放地址
支持 video/note/user/短链接等多种 URL 格式
"""
...
class DouyinParser:
    ...
    def parse(self, url: str) -> dict:
        ...
        # 先尝试直接从原始 URL 提取 video_id（如 user/self?modal_id=xxx 类链接）
        # 避免请求用户主页等需要登录才能访问的页面
        try:
