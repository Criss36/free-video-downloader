...
from downloader import _prepare_cookie_file
...
class SubtitleExtractor:
    ...
    def __init__(self):
        ...
        raw = cookie_path if os.path.isfile(cookie_path) else None
        self.cookie_file = _prepare_cookie_file(raw) if raw else None
    ...
    def extract(self, url: str) -> dict:
        ...
        # 抖音链接：使用 DouyinParser 而非 yt-dlp（yt-dlp 对抖音支持不完善）
    ...
    def _extract_douyin(self, url: str) -> dict:
        """抖音字幕提取：通过 DouyinParser 获取视频信息并检查字幕"""
        empty = {
            "has_subtitle": False, "language": "", "subtitle_type": "none",
            "segments": [], "full_text": "",
        }
        try:
            ...
            item_info = parser._fetch_item_info(video_id, url)
        except Exception:
            return empty
        ...
        # 1. 检查 video_text（视频文案/字幕）
        video_text = item_info.get("video_text")
        if video_text and isinstance(video_text, str):
            segments.append({
                "start": 0.0,
                "end": 0.0,
        ...
        # 2. 检查 chapter_list（章节信息，可能包含字幕）
        chapter_list = item_info.get("chapter_list")
        ...
        # 3. 检查 interaction_stickers（贴纸文本）
        stickers = item_info.get("interaction_stickers")
        if stickers and isinstance(stickers, list):
            for st in stickers:
                text = (st.get("text") or st.get("content") or "").strip()
                if text:
                    segments.append({
                        "start": float(st.get("start_time", 0)),
                        "end": float(st.get("end_time", 0)),
                        "text": text,
                    })
        ...
        full_text = " ".join(seg["text"] for seg in unique)
        return {
            "has_subtitle": True,
            "language": "zh",
            "subtitle_type": "auto",
    ...
    def _get_video_info(self, url: str) -> dict | None:
        try:
